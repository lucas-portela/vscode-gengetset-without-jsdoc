...
const matchers = {
  ...
  privateDef:
    /[\s]*private[\s]*([a-zA-Z_$][0-9a-zA-Z_$]*)[\s]?\:[\s]?([\.\<\>\{\}\[\]a-zA-Z_$\s<>,]+)[\=|\;]/,
  getMethod: /public[\s]get[\s]?([a-zA-Z_$][0-9a-zA-Z_$]*)[\(\)]+/,
  setMethod:
    /public[\s]set[\s]?([a-zA-Z_$][0-9a-zA-Z_$]*)[\(]+[a-zA-Z_$][0-9a-zA-Z_$]*[\s\:]+/,
};
...
export function generateCode(
  classes: IClass[],
  type: EType,
  pickedItem?: vscode.QuickPickItem
) {
  ...
  if (type !== EType.CONSTRUCTOR && pickedItem) {
    ...
    vscode.window.activeTextEditor.edit((builder) => {
      for (let i = 0; i < classes.length; i++) {
        if (
          currentPos.isAfterOrEqual(classes[i].startPos) ||
          currentPos.isBeforeOrEqual(classes[i].endPos)
        ) {
...
export function quickPickItemListFrom(
  classes: IClass[],
  type: EType
    ...
    if (brackets.within) {
      ...
      if (
        ...
        if (_class) {
          ...
          if (
            scoped &&
            (currentPos.isBefore(_class.startPos) ||
              currentPos.isAfter(_class.endPos))
          ) {
            _class.vars = [];
          }
            ...
            for (let i = _class.vars.length - 1; i >= 0; i--) {
              if (type == EType.GETTER || type == EType.BOTH) {
                for (let j = 0; j < _class.getters.length; j++) {
                  ...
                  if (
                    _class.vars[i].figure.toLowerCase() ==
                    _class.getters[j].toLowerCase()
                  ) {
                    _class.vars.splice(i, 1);
                    break;
                  }
                }
              } else if (type == EType.SETTER) {
                for (let j = 0; j < _class.setters.length; j++) {
                  if (
                    _class.vars[i].figure.toLowerCase() ===
                    _class.setters[j].toLowerCase()
                  ) {
                    _class.vars.splice(i, 1);
                    break;
                  }
